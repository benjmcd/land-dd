...
| Empirical qualification | `docs/adr/0004-empirical-qualification-control-plane.md`, `docs/qualification/**`, `config/qualification/**`, `schemas/qualification/**`, `state/EMPIRICAL_QUALIFICATION_STATUS.yaml`, `state/QUALIFICATION_PARAMETERIZATION_BACKLOG.md`, `scripts/validate_qualification.py`, `scripts/selftest_qualification_validator.py`, `plans/2026-06-21-empirical-qualification-adoption.md` | Canonical empirical-validity control plane; the imported spine validates structurally, backlog records blocked owner/source/AOI decisions, readiness/authority gates report into qualification, and no qualification `PASS` exists until owner decisions and empirical evidence are frozen |
