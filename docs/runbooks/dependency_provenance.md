...
The CI `dependency-attestations` job validates those artifacts and, when the repository
has GitHub artifact-attestation entitlement, publishes GitHub artifact attestations for
them and publishes an SBOM attestation that binds the CycloneDX SBOM to the production
lock subject.
...
- the attestation job checks repository entitlement before live publication;
- the attestation job uses `actions/attest@v4` for the lock/SBOM provenance artifacts
  and for the backend dependency SBOM when entitlement is available;
...
6. Treat a failing `dependency-attestations` job as a release blocker until the artifact
   subject paths, SBOM path, GitHub token permissions, entitlement check, or artifact
   metadata write path are corrected.

## Known Limits

- The SBOM is repo-local; CI publishes a GitHub SBOM attestation for it only when
  GitHub artifact-attestation entitlement is available. Private user-owned repositories
  without that entitlement record `Dependency attestations blocked` in the job summary.
- CI publishes GitHub artifact attestations for the lock/SBOM files only when
  entitlement is available. This is not a hosted deployment attestation and does not
  prove a published container image.
