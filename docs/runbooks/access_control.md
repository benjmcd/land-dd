...
| Operator routes | Reviewer-authenticated and scoped | Connector invocation/scheduling requires `connector:run`, connector review decisions require `connector:review`, queue/live-job health reads require `operations:read`, report retry requires `report:retry`, manual approved-connector report creation requires `report:run`, and source-registry mutation requires `source:manage` |
