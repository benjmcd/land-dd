...
| 9 | Public API contract | Contract-ready | `api/openapi_stub.yaml`, `backend/app/api/`, `scripts/export_openapi.py` | Runtime OpenAPI authority, path/method drift guard, report review endpoints, trusted-header report authorization, workspace metadata, scoped idempotency, queued report jobs, explicit worker execution, and bounded operator worker script exist; needs full identity integration and any automatic scheduling decision. |
...
3. Decide full identity integration and automatic report-job scheduling, and
   keep source-failure semantics visible where still missing.
