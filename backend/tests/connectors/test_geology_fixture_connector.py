...
import app.connectors.geology_fixture as geology_fixture_module
from app.connectors import FixtureConnectorError, StaticGeologyFixtureConnector
...
def _write_fixture(tmp_path: Path, payload: dict[str, Any]) -> Path:
    path = tmp_path / "geology-fixture.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
    return path


def test_units_fixture_emits_geology_source_observation() -> None:
    result = StaticGeologyFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_geology_units.json",
    )

    assert result.retrieval_run.connector_name == "fixture_geology_static"
    ...
    assert result.retrieval_run.metrics["geologic_unit_count"] == 2
    ...
    assert evidence.observed_value["geologic_unit_count"] == 2
...
def test_no_units_fixture_emits_geology_no_context_observation() -> None:
    result = StaticGeologyFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_geology_no_units.json",
    )
...
def test_failure_fixture_emits_failed_retrieval_and_source_failure_input() -> None:
    result = StaticGeologyFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_geology_unavailable.json",
    )
    ...
    assert evidence.observed_value["failure_reason"] == "nc_geologic_map_request_error"


def test_connector_rejects_uri_like_fixture_paths() -> None:
    with pytest.raises(FixtureConnectorError, match="local file paths"):
        StaticGeologyFixtureConnector().load_fixture("https://example.test/geology.json")


def test_connector_rejects_connector_name_mismatch(tmp_path: Path) -> None:
    payload = _fixture_payload("nc_buncombe_bun_geology_units.json")
    payload["retrieval_run"]["connector_name"] = "fixture_water_static"

    with pytest.raises(FixtureConnectorError, match="connector_name mismatch"):
        StaticGeologyFixtureConnector().load_fixture(_write_fixture(tmp_path, payload))


def test_connector_rejects_non_geology_evidence(tmp_path: Path) -> None:
    payload = _fixture_payload("nc_buncombe_bun_geology_units.json")
    payload["evidence"][0]["domain"] = "water"

    with pytest.raises(FixtureConnectorError, match="non-geology evidence"):
        StaticGeologyFixtureConnector().load_fixture(_write_fixture(tmp_path, payload))
...
def test_success_fixture_requires_non_failure_source_observation(tmp_path: Path) -> None:
    payload = _fixture_payload("nc_buncombe_bun_geology_unavailable.json")
...
def test_connector_stays_before_claims_reports_and_live_io() -> None:
    source = inspect.getsource(geology_fixture_module)
