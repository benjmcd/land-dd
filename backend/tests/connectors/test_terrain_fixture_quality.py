...
from app.connectors.terrain_fixture import StaticTerrainFixtureConnector
...
def test_terrain_success_fixture_passes_quality_check() -> None:
    connector = StaticTerrainFixtureConnector()
    result = connector.load_fixture(FIXTURE_DIR / "nc_buncombe_bun_slope_terrain.json")
    profile = evaluate_terrain_fixture_quality(result)

    assert profile.passed
    assert profile.connector_name == "fixture_terrain_static"
    assert profile.evidence_count == 1
    assert profile.source_failure_count == 0
    assert profile.blocking_issue_count == 0


def test_terrain_failure_fixture_passes_quality_check() -> None:
    connector = StaticTerrainFixtureConnector()
    result = connector.load_fixture(FIXTURE_DIR / "terrain_failure.json")
    profile = evaluate_terrain_fixture_quality(result)
...
def test_terrain_wrong_connector_name_raises_blocking_issue() -> None:
    result = StaticTerrainFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_terrain.json",
    )
    bad_run = result.retrieval_run.model_copy(
        update={"connector_name": "not_terrain_static"}
    )
    profile = evaluate_terrain_fixture_quality(
...
def test_terrain_succeeded_with_failure_reason_raises_blocking_issue() -> None:
    result = StaticTerrainFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_terrain.json",
    )
    bad_run = result.retrieval_run.model_copy(
...
def test_terrain_evidence_area_id_mismatch_raises_blocking_issue() -> None:
    result = StaticTerrainFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_terrain.json",
    )
...
def test_terrain_evidence_source_id_mismatch_raises_blocking_issue() -> None:
    result = StaticTerrainFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_terrain.json",
    )
    original_evidence = result.evidence_inputs[0]
...
def test_terrain_evidence_observed_before_retrieval_raises_blocking_issue() -> None:
    result = StaticTerrainFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_terrain.json",
    )
...
def test_terrain_evidence_observed_after_retrieval_finished_raises_blocking_issue() -> None:
    result = StaticTerrainFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_terrain.json",
    )
