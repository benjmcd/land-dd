...
def test_parcel_success_fixture_passes_quality_check() -> None:
    connector = StaticParcelFixtureConnector()
    result = connector.load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json"
    )
    profile = evaluate_parcel_fixture_quality(result)

    assert profile.passed
    assert profile.connector_name == "fixture_parcel_static"
    assert profile.evidence_count == 1
    assert profile.source_failure_count == 0
    assert profile.blocking_issue_count == 0


def test_parcel_failure_fixture_passes_quality_check() -> None:
    connector = StaticParcelFixtureConnector()
    result = connector.load_fixture(FIXTURE_DIR / "parcel_failure.json")
    profile = evaluate_parcel_fixture_quality(result)
...
def test_parcel_brunswick_fixture_passes_quality_check() -> None:
    connector = StaticParcelFixtureConnector()
    result = connector.load_fixture(
        FIXTURE_DIR / "nc_brunswick_bru_jurisdiction_parcels.json"
    )
    profile = evaluate_parcel_fixture_quality(result)
...
def test_parcel_wrong_connector_name_raises_blocking_issue() -> None:
    result = StaticParcelFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json",
    )
    bad_run = result.retrieval_run.model_copy(
        update={"connector_name": "not_parcel_static"}
    )
    profile = evaluate_parcel_fixture_quality(
...
def test_parcel_succeeded_with_failure_reason_raises_blocking_issue() -> None:
    result = StaticParcelFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json",
    )
    bad_run = result.retrieval_run.model_copy(
...
def test_parcel_evidence_area_id_mismatch_raises_blocking_issue() -> None:
    result = StaticParcelFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json",
    )
...
def test_parcel_evidence_source_id_mismatch_raises_blocking_issue() -> None:
    result = StaticParcelFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json",
    )
    original_evidence = result.evidence_inputs[0]
...
def test_parcel_evidence_observed_before_retrieval_raises_blocking_issue() -> None:
    result = StaticParcelFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json",
    )
...
def test_parcel_evidence_observed_after_retrieval_finished_raises_blocking_issue() -> None:
    result = StaticParcelFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json",
    )
...
def test_parcel_missing_spatial_geometry_raises_blocking_issue() -> None:
    result = StaticParcelFixtureConnector().load_fixture(
        FIXTURE_DIR / "nc_buncombe_bun_slope_parcels.json",
    )
    bad_evidence = result.evidence_inputs[0].model_copy(
