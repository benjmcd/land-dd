...
def _area(area_id: UUID) -> AreaContract:
    return AreaContract(
        area_id=area_id,
        label="Brunswick parcels API test area",
        geom_geojson={
            "type": "Polygon",
            "coordinates": [
                [
...
def _client_with_seeded_services(
    *,
    ...
    services.brunswick_parcels_fetch_json = fetch_json
    return TestClient(app), services, area_id


def test_brunswick_parcels_query_bbox_returns_202_and_persists_evidence() -> None:
    client, services, area_id = _client_with_seeded_services(
        fetch_payload={"type": "FeatureCollection", "features": [_feature()]},
    )

    response = client.post(
        "/connector-runs/brunswick-parcels/query-bbox",
        json=_body(area_id),
    ...
    body = response.json()
    assert body["connector_name"] == "brunswick_parcels_live"
    ...
    assert evidence[0].observed_value.get("parcel_pin") == "1122334455"


def test_brunswick_parcels_query_bbox_empty_response_emits_source_failure() -> None:
    client, services, area_id = _client_with_seeded_services(
    ...
    assert evidence[0].observed_value.get("failure_reason") == "brunswick_parcels_no_features"


def test_brunswick_parcels_query_bbox_connection_error_emits_source_failure() -> None:
    client, services, area_id = _client_with_seeded_services(
    ...
    assert len(evidence) == 1
    assert evidence[0].evidence_type == EvidenceType.SOURCE_FAILURE
    assert evidence[0].observed_value.get("failure_reason") == "brunswick_parcels_request_error"


def test_brunswick_parcels_query_bbox_rejects_unregistered_area() -> None:
    ...
    response = client.post(
        ...
        json={
            ...
            "bbox": {"xmin": -78.30, "ymin": 34.00, "xmax": -78.10, "ymax": 34.20},
            "max_features": 10,
        },
        headers=_VALID_HEADERS,
    )
...
def test_brunswick_parcels_query_bbox_rejects_missing_auth() -> None:
