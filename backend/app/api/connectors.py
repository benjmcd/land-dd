...
from app.connectors.noaa_climate import NoaaClimateBbox, NoaaClimateConnectorError
from app.connectors.nwi import NWI_MAX_FEATURES, NwiBbox
from app.connectors.osm_road_access import OsmRoadAccessConnectorError
...
class OsmRoadAccessBboxRequest(BaseModel):
    xmin: float
    ymin: float
    xmax: float
    ymax: float
...
class OsmRoadAccessQueryRequest(BaseModel):
    area_id: UUID
    bbox: OsmRoadAccessBboxRequest
...
class ChathamParcelsBboxRequest(BaseModel):
    xmin: float
    ymin: float
    xmax: float
    ymax: float
...
class ChathamParcelsQueryRequest(BaseModel):
    area_id: UUID
...
@router.post(
    "/osm-road-access/query-bbox",
    response_model=OsmRoadAccessQueryResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def query_osm_road_access_bbox(
    ...
    try:
        ...
        result = orchestrate_osm_road_access_for_area(
            services=services,
            area=area_for_bbox,
            max_features=request.max_features,
        )
...
class FccBroadbandBboxRequest(BaseModel):
    xmin: float
    ymin: float
    xmax: float
    ymax: float
...
class FccBroadbandQueryRequest(BaseModel):
    area_id: UUID
    bbox: FccBroadbandBboxRequest


class FccBroadbandQueryResponse(BaseModel):
    connector_name: str
    ingest_run_id: UUID
    retrieval_status: str
    row_count: int | None
...
@router.post(
    "/fcc-broadband/query-bbox",
    response_model=FccBroadbandQueryResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def query_fcc_broadband_bbox(
    ...
    try:
        ...
        result = orchestrate_fcc_broadband_for_area(
            services=services,
            area=area_for_bbox,
        )
        queue_item = result.queue_item
    ...
    return {
        ...
        "queue_item_status": queue_item.status.value,
        "queue_name": handoff.queue_name,
        "source_registry_id": DS_021_REGISTRY_ID,
        "request_url": result.request_url,
    }


class BlmMlrsBboxRequest(BaseModel):
    xmin: float
    ymin: float
    xmax: float
    ymax: float
...
class BlmMlrsQueryRequest(BaseModel):
    area_id: UUID
    bbox: BlmMlrsBboxRequest
    max_features: int = BLM_MLRS_MAX_FEATURES


class BlmMlrsQueryResponse(BaseModel):
    connector_name: str
    ingest_run_id: UUID
    retrieval_status: str
    row_count: int | None
...
class UsgsMrdsBboxRequest(BaseModel):
    xmin: float
    ymin: float
    xmax: float
    ymax: float
...
class UsgsMrdsQueryRequest(BaseModel):
    area_id: UUID
    bbox: UsgsMrdsBboxRequest
    max_features: int = USGS_MRDS_MAX_FEATURES


class UsgsMrdsQueryResponse(BaseModel):
    connector_name: str
    ingest_run_id: UUID
    retrieval_status: str
    row_count: int | None
...
class NcGeologicMapBboxRequest(BaseModel):
    xmin: float
    ymin: float
    xmax: float
    ymax: float
...
class NcGeologicMapQueryRequest(BaseModel):
    area_id: UUID
    bbox: NcGeologicMapBboxRequest
    max_features: int = NC_GEOLOGIC_MAP_MAX_FEATURES


class NcGeologicMapQueryResponse(BaseModel):
    connector_name: str
    ingest_run_id: UUID
    retrieval_status: str
    row_count: int | None
...
@router.post(
    "/blm-mlrs/query-bbox",
    response_model=BlmMlrsQueryResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def query_blm_mlrs_bbox(
    ...
    try:
        ...
        result = orchestrate_blm_mlrs_for_area(
            services=services,
            area=area_for_bbox,
            max_features=request.max_features,
        )
    ...
    return {
        ...
        "queue_item_status": queue_item.status.value,
        "queue_name": handoff.queue_name,
        "source_registry_id": DS_007_REGISTRY_ID,
        "request_url": result.request_url,
    }


@router.post(
    "/usgs-mrds/query-bbox",
    response_model=UsgsMrdsQueryResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def query_usgs_mrds_bbox(
    ...
    try:
        ...
        result = orchestrate_usgs_mrds_for_area(
            services=services,
            area=area_for_bbox,
            max_features=request.max_features,
        )
    ...
    return {
        ...
        "queue_item_status": queue_item.status.value,
        "queue_name": handoff.queue_name,
        "source_registry_id": DS_008_REGISTRY_ID,
        "request_url": result.request_url,
    }


@router.post(
    "/nc-geologic-map/query-bbox",
    response_model=NcGeologicMapQueryResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
def query_nc_geologic_map_bbox(
    ...
    try:
        ...
        result = orchestrate_nc_geologic_map_for_area(
            services=services,
            area=area_for_bbox,
            max_features=request.max_features,
        )
    ...
    return {
        ...
        "queue_item_status": queue_item.status.value,
        "queue_name": handoff.queue_name,
        "source_registry_id": DS_015_REGISTRY_ID,
        "request_url": result.request_url,
    }


class NoaaClimateBboxRequest(BaseModel):
    xmin: float
    ymin: float
    xmax: float
    ymax: float
...
class NoaaClimateQueryRequest(BaseModel):
    area_id: UUID
    bbox: NoaaClimateBboxRequest


class NoaaClimateQueryResponse(BaseModel):
    connector_name: str
    ingest_run_id: UUID
    retrieval_status: str
    row_count: int | None
...
def query_noaa_climate_bbox(
    ...
    try:
        ...
        result = orchestrate_noaa_climate_for_area(
            services=services,
            area=area_for_bbox,
        )
        queue_item = result.queue_item
...
@router.post(
    "/chatham-parcels/query-bbox",
    response_model=ChathamParcelsQueryResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
