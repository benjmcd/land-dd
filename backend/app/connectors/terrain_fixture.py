...
class TerrainFixtureConnectorResult:
    retrieval_run: SourceRetrievalRunContract
    evidence_inputs: tuple[EvidenceContract, ...]


class StaticTerrainFixtureConnector:
    connector_name = "fixture_terrain_static"
    domain = "terrain"

    def load_fixture(self, fixture_path: str | Path) -> TerrainFixtureConnectorResult:
        ...
        return TerrainFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        if not evidence_inputs:
            raise FixtureConnectorError("terrain fixture must emit evidence inputs")
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError(
                    "terrain connector emitted non-terrain evidence"
                )
        if retrieval_run.status == SourceRetrievalStatus.SUCCEEDED:
            if not any(
                ...
                raise FixtureConnectorError(
                    "successful terrain fixture must emit derived-metric evidence"
                )
            return
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked terrain fixture must emit source-failure evidence"
                )
            return
        raise FixtureConnectorError(
            "terrain fixture retrieval status must be succeeded, failed, or blocked"
        )
