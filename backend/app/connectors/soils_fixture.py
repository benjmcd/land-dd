...
class SoilsFixtureConnectorResult:
    retrieval_run: SourceRetrievalRunContract
    evidence_inputs: tuple[EvidenceContract, ...]


class StaticSoilsFixtureConnector:
    connector_name = "fixture_soils_static"
    domain = "soils"

    def load_fixture(self, fixture_path: str | Path) -> SoilsFixtureConnectorResult:
        ...
        return SoilsFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        if not evidence_inputs:
            raise FixtureConnectorError("soils fixture must emit evidence inputs")
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError(
                    "soils connector emitted non-soils evidence"
                )
        if retrieval_run.status == SourceRetrievalStatus.SUCCEEDED:
            if not any(
                ...
                raise FixtureConnectorError(
                    "successful soils fixture must emit spatial intersection evidence"
                )
            return
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked soils fixture must emit source-failure evidence"
                )
            return
        raise FixtureConnectorError(
            "soils fixture retrieval status must be succeeded, failed, or blocked"
        )
