...
class ParcelFixtureConnectorResult:
    retrieval_run: SourceRetrievalRunContract
    evidence_inputs: tuple[EvidenceContract, ...]


class StaticParcelFixtureConnector:
    connector_name = "fixture_parcel_static"
    domain = "parcels"

    def load_fixture(self, fixture_path: str | Path) -> ParcelFixtureConnectorResult:
        ...
        return ParcelFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        if not evidence_inputs:
            raise FixtureConnectorError("parcel fixture must emit evidence inputs")
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError(
                    "parcel connector emitted non-parcel evidence"
                )
        if retrieval_run.status == SourceRetrievalStatus.SUCCEEDED:
            if not any(
                ...
                raise FixtureConnectorError(
                    "successful parcel fixture must emit spatial intersection evidence"
                )
            return
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked parcel fixture must emit source-failure evidence"
                )
            return
        raise FixtureConnectorError(
            "parcel fixture retrieval status must be succeeded, failed, or blocked"
        )
