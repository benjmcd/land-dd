...
class MineralsFixtureConnectorResult:
...
class StaticMineralsFixtureConnector:
    ...
    connector_name = "fixture_minerals_static"
    domain = "minerals"

    def load_fixture(self, fixture_path: str | Path) -> MineralsFixtureConnectorResult:
        ...
        return MineralsFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError(
                    "minerals connector emitted non-minerals evidence",
                )
        ...
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked minerals fixture must emit source-failure evidence",
                )
            return

        raise FixtureConnectorError(
            "minerals fixture retrieval status must be succeeded, failed, or blocked",
        )
