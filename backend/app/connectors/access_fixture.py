...
class AccessFixtureConnectorResult:
    retrieval_run: SourceRetrievalRunContract
    evidence_inputs: tuple[EvidenceContract, ...]


class StaticAccessFixtureConnector:
    connector_name = "fixture_access_static"
    domain = "access"

    def load_fixture(self, fixture_path: str | Path) -> AccessFixtureConnectorResult:
        ...
        return AccessFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError("access connector emitted non-access evidence")
        ...
        if retrieval_run.status == SourceRetrievalStatus.SUCCEEDED:
            if not any(
                ...
                raise FixtureConnectorError(
                    "successful access fixture must emit spatial evidence",
                )
            return
        ...
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked access fixture must emit source-failure evidence",
                )
            return

        raise FixtureConnectorError(
            "access fixture retrieval status must be succeeded, failed, or blocked",
        )
