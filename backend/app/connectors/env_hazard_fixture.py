...
class EnvHazardFixtureConnectorResult:
    retrieval_run: SourceRetrievalRunContract
    evidence_inputs: tuple[EvidenceContract, ...]


class StaticEnvHazardFixtureConnector:
    """Fixture connector for EPA ECHO environmental hazard evidence."""

    connector_name = "fixture_env_hazard_static"
    domain = "env_hazard"

    def load_fixture(self, fixture_path: str | Path) -> EnvHazardFixtureConnectorResult:
        ...
        return EnvHazardFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError(
                    "env-hazard connector emitted non-env-hazard evidence",
                )
        ...
        if retrieval_run.status == SourceRetrievalStatus.SUCCEEDED:
            ...
            if not non_failure:
                raise FixtureConnectorError(
                    "successful env-hazard fixture must emit non-failure evidence",
                )
            if not all(
                ...
                raise FixtureConnectorError(
                    "successful env-hazard fixture non-failure evidence must be "
                    "source observations",
                )
            return
        ...
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked env-hazard fixture must emit source-failure evidence",
                )
            return

        raise FixtureConnectorError(
            "env-hazard fixture retrieval status must be succeeded, failed, or blocked",
        )
