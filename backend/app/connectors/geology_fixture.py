...
class GeologyFixtureConnectorResult:
    retrieval_run: SourceRetrievalRunContract
    evidence_inputs: tuple[EvidenceContract, ...]


class StaticGeologyFixtureConnector:
    """Fixture connector for NC geologic map unit context evidence."""

    connector_name = "fixture_geology_static"
    domain = "geology"

    def load_fixture(self, fixture_path: str | Path) -> GeologyFixtureConnectorResult:
        ...
        return GeologyFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError(
                    "geology connector emitted non-geology evidence",
                )
        ...
        if retrieval_run.status == SourceRetrievalStatus.SUCCEEDED:
            ...
            if not non_failure:
                raise FixtureConnectorError(
                    "successful geology fixture must emit non-failure evidence",
                )
            if not all(
                ...
                raise FixtureConnectorError(
                    "successful geology fixture non-failure evidence must be "
                    "source observations",
                )
            return
        ...
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked geology fixture must emit source-failure evidence",
                )
            return

        raise FixtureConnectorError(
            "geology fixture retrieval status must be succeeded, failed, or blocked",
        )
