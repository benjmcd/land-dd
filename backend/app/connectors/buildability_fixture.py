...
class BuildabilityFixtureConnectorResult:
    retrieval_run: SourceRetrievalRunContract
    evidence_inputs: tuple[EvidenceContract, ...]


class StaticBuildabilityFixtureConnector:
    connector_name = "fixture_buildability_static"
    domain = "buildability"

    def load_fixture(self, fixture_path: str | Path) -> BuildabilityFixtureConnectorResult:
        ...
        return BuildabilityFixtureConnectorResult(
    ...
    def _validate_result(
        ...
        if not evidence_inputs:
            raise FixtureConnectorError("buildability fixture must emit evidence inputs")
        for evidence in evidence_inputs:
            if evidence.domain != self.domain:
                raise FixtureConnectorError(
                    "buildability connector emitted non-buildability evidence"
                )
        if retrieval_run.status == SourceRetrievalStatus.SUCCEEDED:
            if not any(
                evidence.evidence_type == EvidenceType.DERIVED_METRIC
                and not evidence.is_source_failure
                for evidence in evidence_inputs
            ):
                raise FixtureConnectorError(
                    "successful buildability fixture must emit derived-metric evidence"
                )
            return
        if retrieval_run.status in {
            ...
            if not any(
                ...
                raise FixtureConnectorError(
                    "failed or blocked buildability fixture must emit source-failure evidence"
                )
            return
        raise FixtureConnectorError(
            "buildability fixture retrieval status must be succeeded, failed, or blocked"
        )
