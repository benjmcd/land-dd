...
foreach ($file in $requiredFiles) {
    if (-not (Test-Path -Path (Join-Path $root $file) -PathType Leaf)) {
        throw "required alerting artifact missing: $file"
    }
}
...
def require_existing(path_text: str) -> None:
    ...
    require(path.exists(), f"referenced alerting artifact missing: {normalized}")
...
def validate_source_readiness() -> None:
    ...
    require("ready_count" in payload and "blocked_count" in payload, "source readiness counts missing")
...
if (Get-Command docker -ErrorAction SilentlyContinue) {
    ...
    Write-Host 'alert rules check: docker unavailable; compose config skipped'
}
