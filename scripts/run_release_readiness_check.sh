...
require(source_readiness.get("ready_count") == 7, "Must ready count changed")
require(source_readiness.get("blocked_count") == 1, "Must blocked count changed")
blocked = {
    str(source.get("source_registry_id"))
    for source in source_readiness.get("sources", [])
    if source.get("connector_ready") is False
}
require(blocked == {"DS-017"}, "expected source blocker set changed")
