...
def _check_sql_seed_rows(rows: list[dict[str, str]], errors: list[str]) -> None:
    ...
    for row in rows:
        ...
        if source_id not in sql_rows:
            errors.append(f"{SQL_SEED_PATH.relative_to(ROOT)} missing source {source_id}")
            continue

        sql_row = sql_rows[source_id]
        metadata = sql_row["metadata"]
        _compare_sql_value(source_id, "Commercial Use Status", row, sql_row, errors)
        _compare_sql_attribution_required(source_id, row, sql_row, errors)
...
def _compare_sql_attribution_required(
    source_id: str,
    row: dict[str, str],
    sql_row: dict[str, object],
    errors: list[str],
) -> None:
    expected = "true" if row["Attribution Required"].strip() == "yes" else "false"
    actual = str(sql_row["attribution_required"]).strip()
    if expected != actual:
        errors.append(
            f"{SQL_SEED_PATH.relative_to(ROOT)} {source_id} attribution_required="
...
def _load_sql_seed_rows() -> dict[str, dict[str, Any]]:
    ...
    for line in SQL_SEED_PATH.read_text(encoding="utf-8").splitlines():
        ...
        metadata = cast(
            dict[str, object],
            json.loads(str(parsed[13]).removesuffix("::jsonb")),
        )
        source_id = str(metadata.get("source_registry_id", ""))
        if source_id:
            rows[source_id] = {
                "commercial_use_status": parsed[6],
                "attribution_required": parsed[8],
                "ai_use_allowed": parsed[9],
                "cache_allowed": parsed[10],
                "export_allowed": parsed[11],
                "raw_data_allowed": parsed[12],
                "metadata": metadata,
            }
    return rows
