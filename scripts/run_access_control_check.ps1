...
$requiredFiles = @(
    ...
    'backend\tests\api\test_api_key_auth.py',
    'backend\tests\api\test_reviewer_auth.py'
)

foreach ($file in $requiredFiles) {
    if (-not (Test-Path -Path (Join-Path $root $file) -PathType Leaf)) {
        throw "required access-control artifact missing: $file"
    }
}
...
def validate_catalog() -> None:
    payload = yaml.safe_load((ROOT / "config" / "access_control.yaml").read_text(encoding="utf-8"))
    require(isinstance(payload, dict), "access-control catalog must be a mapping")
    require(payload.get("schema_version") == "access_control_v1", "unexpected access-control schema")
    require(payload.get("operator_runbook") == "docs/runbooks/access_control.md", "operator runbook mismatch")
    ...
    non_goals = payload.get("local_only_non_goals")
...
def validate_api_key_auth() -> None:
    auth = (ROOT / "backend" / "app" / "api" / "api_key_auth.py").read_text(encoding="utf-8")
    ...
    tests = (ROOT / "backend" / "tests" / "api" / "test_api_key_auth.py").read_text(encoding="utf-8")
    ...
    for phrase in (
        ...
        require(phrase in auth, f"api key auth missing phrase: {phrase}")
    for phrase in (
        "ApiKeyAuthAuditEvent",
    ...
    for phrase in (
        ...
        require(phrase in tests, f"api key tests missing phrase: {phrase}")


def validate_reviewer_auth() -> None:
    auth = (ROOT / "backend" / "app" / "api" / "reviewer_auth.py").read_text(encoding="utf-8")
    tests = (ROOT / "backend" / "tests" / "api" / "test_reviewer_auth.py").read_text(encoding="utf-8")
    settings = (ROOT / "backend" / "app" / "core" / "config.py").read_text(encoding="utf-8")
    for phrase in (
        ...
        require(phrase in auth, f"reviewer auth missing phrase: {phrase}")
    for phrase in (
    ...
    for phrase in (
        ...
        require(phrase in tests, f"reviewer auth tests missing phrase: {phrase}")
...
def validate_ci_and_runbook() -> None:
    ...
    require("actions/checkout@v4" in text, "access-control job must checkout repo")
    require("actions/setup-python@v5" in text, "access-control job must setup Python")
    ...
    require("./scripts/run_access_control_check.sh" in text, "access-control job must run POSIX access-control proof")
