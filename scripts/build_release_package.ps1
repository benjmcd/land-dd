...
def iter_release_files(
    ...
    require(files, "release package would be empty")
    return files
...
def main() -> int:
    version = normalized_version(sys.argv[1] if len(sys.argv) > 1 else "")
    config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    require(isinstance(config, dict), "release package config must be a mapping")
    require(config.get("schema_version") == "release_package_v1", "unexpected release package schema")
    package_name = config.get("package_name")
    require(isinstance(package_name, str) and package_name, "package_name is required")
    output_dir_text = config.get("output_dir")
    require(isinstance(output_dir_text, str) and output_dir_text, "output_dir is required")
    manifest_name = config.get("manifest_filename")
    require(isinstance(manifest_name, str) and manifest_name.endswith(".json"), "manifest_filename must be a JSON file")
