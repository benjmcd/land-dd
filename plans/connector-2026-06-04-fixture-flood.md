...
## CON-038 Connector Fixture Source-Failure Geometry Absence

CON-038 tightens connector-local fixture quality around source-failure evidence shape.

### Implemented Checks

Source-failure fixture evidence must not carry `geometry_geojson` or `spatial_precision_meters`. Source failures represent retrieval/source unavailability, not a spatial observation; spatial geometry remains required only for non-failure spatial intersection evidence.
...
Result: pass on 2026-06-04. Focused fixture-quality tests cover 20 cases; focused ruff clean; focused mypy clean over 2 source files. Full DB-enabled Windows PowerShell verification passes with 372 backend tests collected/passing, lint clean, mypy clean over 123 source files, migrations/seeds applied, and DB smoke passed.

### Next Slice

After Session 1's Lane C evidence-linkage/OpenAPI branch lands or a clean merge point is coordinated, implement `POST /connector-runs/{ingest_run_id}/review-actions` for the accepted subset above and refresh OpenAPI parity in the same route implementation slice. If that branch remains parked, continue with non-OpenAPI connector fixture-quality, report metadata, or schema-boundary work.

## CON-037 Connector Fixture Method-Code Consistency

CON-037 tightens connector-local fixture quality around flood fixture method provenance.

### Implemented Checks

Non-empty fixture evidence `method_code` values must start with `fixture_flood_`. Empty method codes remain covered by the existing evidence provenance text check; non-flood fixture method codes are rejected before output can be accepted as flood fixture evidence.
...
Result: pass on 2026-06-04. Focused fixture-quality tests cover 19 cases; focused ruff clean; focused mypy clean over 2 source files. Full DB-enabled Windows PowerShell verification passes with 371 backend tests collected/passing, lint clean, mypy clean over 123 source files, migrations/seeds applied, and DB smoke passed.

### Next Slice

After Session 1's Lane C evidence-linkage/OpenAPI branch lands or a clean merge point is coordinated, implement `POST /connector-runs/{ingest_run_id}/review-actions` for the accepted subset above and refresh OpenAPI parity in the same route implementation slice. If that branch remains parked, continue with non-OpenAPI connector fixture-quality, report metadata, or schema-boundary work.

## CON-036 Connector Fixture Source-Failure Type Consistency

CON-036 tightens connector-local fixture quality around source-failure evidence typing.

### Implemented Checks

Fixture evidence must keep the `is_source_failure` flag aligned with `evidence_type == "source_failure"`. Mismatched source-failure flags and evidence types are rejected before the fixture output can be accepted.
...
Result: pass on 2026-06-04. Focused fixture-quality tests cover 18 cases; focused ruff clean; focused mypy clean over 2 source files. Full DB-enabled Windows PowerShell verification passes with 370 backend tests collected/passing, lint clean, mypy clean over 123 source files, migrations/seeds applied, and DB smoke passed.
