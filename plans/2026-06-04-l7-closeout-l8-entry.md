...
|---|---|---|---|
| `schemas/source_schema.json` | Resolved for serialized `SourceContract`: field set and `authority_level` enum are parity-tested against Lane A contracts. Source dataset/version/retrieval-run contracts are covered separately by `schemas/source_provenance_schema.json`. | Lane A | Keep `source_schema.json` scoped to source identity/governance only. |
| `schemas/source_provenance_schema.json` | Resolved for serialized `SourceDatasetContract`, `SourceDatasetVersionContract`, and `SourceRetrievalRunContract`: field sets and retrieval status enum values are parity-tested against Lane A contracts. | Lane A | Future edits should only add runtime validation or new provenance-family fields through a Lane A/coordinator plan. |
...
| `schemas/job_schema.json` | Matches the broad `JobStatus` values but there is no current `JobContract`; Level 8 connector/retrieval run statuses also use `SourceRetrievalStatus` values (`pending`, `running`, `succeeded`, `failed`, `blocked`, `skipped`) that do not fully match job status. ADR Lane D 0018 records that the schema is not yet a live connector-run/API contract and must choose `jobs.job_queue`, `ConnectorReviewQueueItem`, or a future `JobContract` before edits. | Coordinator with Lane A/Lane D | Future schema edits must keep jobs as orchestration state and preserve source retrieval runs as connector provenance authority. |
| `schemas/report_run_schema.json` | Resolved for serialized `ReportRunContract`: field set, required fields, `intent_code`, `status`, and nested evidence/claim schema references are parity-tested. Stable generated `source_manifest`, `source_details`, `artifact_metadata`, `cost_metrics`, and optional `artifact_metadata.validation` keys are now constrained while extension fields remain open. Report metadata extension boundary is accepted by ADR Lane D 0013; the first validation metadata implementation is accepted by ADR Lane D 0017. | Lane D | Future edits should choose a specific accepted metadata extension family, job schema, or OpenAPI output envelope before schema/runtime changes. |
...
5. Level 8 connector work continues with fixture-only connector contracts that reference aligned source/evidence schemas and explicitly record any durable evidence-row lineage gaps.
...
- 2026-06-04: Lane D TD-082 accepted the report metadata extension boundary. Source provenance-family schemas are aligned; job schema, API mutation/workflow implementation, and durable evidence-row retrieval lineage remain future coordinated work.
- 2026-06-04: Lane D TD-083 implemented optional `artifact_metadata.validation` for report contract/profile and ruleset identity. Job schema, API mutation/workflow implementation, and durable evidence-row retrieval lineage remain future coordinated work.
- 2026-06-04: Lane D TD-084 accepted the job schema boundary. `schemas/job_schema.json` remains unedited and is not promoted to a live connector-run/API contract until a future schema/test slice chooses its authority.
