...
| `schemas/report_run_schema.json` | Resolved for serialized `ReportRunContract`: field set, required fields, `intent_code`, `status`, and nested evidence/claim schema references are parity-tested. Stable generated `source_manifest`, `source_details`, `artifact_metadata`, and `cost_metrics` keys are now constrained while extension fields remain open. Report metadata extension boundary is accepted by ADR Lane D 0013. | Lane D | Future edits should choose a specific accepted metadata extension family, job schema, or OpenAPI output envelope before schema/runtime changes. |
