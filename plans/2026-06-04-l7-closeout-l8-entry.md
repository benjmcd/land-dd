...
| `schemas/report_run_schema.json` | Resolved for serialized `ReportRunContract`: field set, required fields, `intent_code`, `status`, and nested evidence/claim schema references are parity-tested. `source_manifest` and `artifact_metadata` remain open objects pending future report manifest metadata decisions. | Lane D | Decide whether to tighten source manifest and artifact metadata maps after connector provenance, cost metrics, and report artifact metadata semantics stabilize. |
...
3. Lane D proposes `schemas/report_run_schema.json` only after nested source/evidence/claim schema scope is settled. Status: complete for serialized `ReportRunContract`.
...
- 2026-06-04: Lane D TD-080 added `schemas/report_run_schema.json` for serialized `ReportRunContract` and parity tests. Report source-manifest/artifact-metadata tightening, source provenance-family schemas, job schema, and OpenAPI refresh remain future work.
