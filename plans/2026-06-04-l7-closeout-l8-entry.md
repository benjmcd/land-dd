...
| `schemas/report_run_schema.json` | Resolved for serialized `ReportRunContract`: field set, required fields, `intent_code`, `status`, and nested evidence/claim schema references are parity-tested. Stable generated `source_manifest`, `source_details`, `artifact_metadata`, and `cost_metrics` keys are now constrained while extension fields remain open. | Lane D | Future edits should decide only new report metadata extensions, job schema, or OpenAPI output envelopes. |
