...
- `backend/app/evidence_ledger/` and `backend/app/claims_engine/` module directories exist (empty).
- `backend/tests/evidence_ledger/` and `backend/tests/claims_engine/` test directories exist.
- 6 tests in `backend/tests/evidence_ledger/test_evidence_contracts.py` and `backend/tests/claims_engine/test_claim_contracts.py`.
...
4. `create_observation(evidence: EvidenceContract) -> EvidenceContract`: validates source registered + area registered, stores.
5. `create_source_failure(area_id, source_id, method_code, caveat) -> EvidenceContract`: creates `is_source_failure=True` record.
6. `get(evidence_id)`, `list_by_area(area_id)`.
7. Tests: happy path, source failure, source-not-registered rejection, area-not-registered rejection.
