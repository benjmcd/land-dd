...
## Current checkpoint (2026-07-02 Post-169 env-hazard fixture routing)

This branch starts from live `origin/main`
`3b2bd1897f8227ca24cb7290d8026ff3b2757356`, after the ODGAV owner-answer gate
evaluation merged through PR #167 and the first extended-domain minerals
fixture-ingestion proof merged through PR #168, and the broadband fixture-ingestion
proof merged through PR #169. The active pass is owner-independent US-MVP fixture
ingestion only: extend the landed minerals/broadband pattern to EPA ECHO
environmental hazard fixture evidence on the existing Buncombe golden AOI. It does not
run live EPA ECHO calls, approve sources, change source rights, seed or mutate the DB
outside the in-memory/private-MVP test path, change schema/API/auth/report semantics,
approve DS-017, unfreeze qualification, record Bologna authority, claim hosted/Level 10
authority, or change `P0 = BLOCKED`.

- **Current implementation plan**:
  `plans/2026-07-02-env-fixture.md`.
- **Known boundaries to preserve**:
  The environmental hazard fixture-ingestion pass is a repo-local, owner-independent utility
...
- **Current extended-domain boundary**:
  ...
  complete through PR #169. This branch extends that pattern to environmental hazards
  with local EPA ECHO fixtures, connector fail-closed tests, and private-MVP
  regulated-facility/source-failure tests on the existing Buncombe golden AOI. Water
  and geology remain later lanes until this env-hazard proof is reviewed/landed and
  the next fixture-ingestion plan is selected.
- **Current task state**:
  Active task is ENV-FIXTURE. Completed task lineage retained from prior
  ...
  `BROADBAND-FIXTURE`. `BSA-001` remains blocked until product/AOI and source-review
  authority is cited. `P0` remains `BLOCKED`.
- **Immediate next pursuit**: implement and validate the environmental hazard
  fixture-ingestion lane through branch review/merge. Acceptance proves regulated
  facility and source-failure evidence paths through local fixture ingestion,
  evidence-to-claim linkage, Section 11 dossier rendering, and forbidden-overclaim
  checks while preserving all Bologna,
