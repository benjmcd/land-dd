...
Milestone status: PARTIAL PASS for local-only Level 10 production hardening: scoped local reviewer auth, default-off API-key middleware, static local key lifecycle specs, auth audit logs/events, rate limiting, structured logs/metrics, Docker/Compose local runtime smoke, backup/restore proof, local alert-rule/catalog checks, supply-chain/dependency/container scan proofs, local release package proof, access-control posture proof, bounded reviewed DS-001/DS-002/DS-003/DS-004 connector workflows, review-gated connector evidence, report retry/lineage/compare/diff surfaces, and Postgres-backed async report job state are implemented. Billing, hosted deployment, hosted deployment attestation, registry push/signing, published registry-image attestation, automatic key rotation/external secret-manager integration, hosted log retention/alerting, and full user auth/RBAC/OIDC/user accounts are out of scope for local-only operation; remaining active blocker is non-ready Must-source review beyond DS-001/DS-002/DS-003/DS-004.
...
- docker compose ps
Verification result:
- Level 10 local-only hardening slice passes for current implemented local controls and workflows. Remote/hosted/billing/user-account requirements are now recorded as `out_of_scope_local_only`, not local-only release blockers. Current active release blocker remains source readiness for non-DS-001/DS-002/DS-003/DS-004 Must sources until those source-rights reviews are complete.
...
- Remaining achievable L10 work: review remaining blocked `Must` sources, strengthen local
  load/performance evidence, wire operator report export (PDF/JSON artifact download), or add
  Web UI improvements for the operator workflow. Hosted auth/RBAC, billing, hosted
  log retention, hosted deployment, and registry publication are deferred out of scope.
Do not work on yet:
...
Latest US-073 through US-082 verification on 2026-06-05: load test baseline, security scan, data retention policy catalog, jurisdiction/rulepack checklists, DB pool config, performance runbook, lineage/compare/diff APIs, and full local verification passed. Remaining local-only L10 blocker before US-083 was non-ready Must sources; prior hosted/auth/billing items are superseded by US-083 as out-of-scope for local-only operation.

Latest US-083 local-only scope correction on 2026-06-06: `MILESTONE_MAP.md`, `docs/PRODUCT_SPEC.md`, `MANIFEST.md`, `config/release_readiness.yaml`, `config/access_control.yaml`, `config/image_publication.yaml`, `config/hosted_deployment.yaml`, `config/data_retention.yaml`, `config/ops_cost_monitoring.yaml`, runbooks, validation scripts, artifact tests, and the active L10 plan now classify billing/hosted billing, hosted deployment and hosted attestation, registry push/signing and published registry-image attestation, automatic key rotation/external secret-manager integration, hosted log retention/alerting, and full user auth/RBAC/OIDC/user accounts as `out_of_scope_local_only`. `release_blockers` now keeps `non_ready_must_sources` as the active local-only blocker, and CI no longer runs remote-only `image-publication` or `hosted-deployment` jobs for the local-only release path. Verification for this slice is recorded in `state/VALIDATION_LOG.md`.
