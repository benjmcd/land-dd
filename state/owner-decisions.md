...
Validator-compatibility correction: frozen flood unknown_states = [SOURCE_FAILED, OUTSIDE_COVERAGE, STALE, CONFLICTING]. The 2026-07-06 owner authorization ratified state/qfreeze2-flood-proposal.md AS-IS while that packet still listed the catch-all UNKNOWN in unknown_states; because validate_qualification.py unresolved() rejects the bare token UNKNOWN in a FROZEN record, the AS-IS packet could not be applied as written. The catch-all UNKNOWN was removed as a validator-compatibility correction under ORCHESTRATOR authorization on 2026-07-06 (a disclosed deviation from the AS-IS authorization), not a separate owner value-level directive. The correction preserves the owner's fail-closed intent and changes no threshold, scope, or safety posture; the four explicit states retain fail-closed handling.

Stop-and-record reconciliation: the bare-UNKNOWN removal was surfaced as an explicit implementation blocker before the freeze was applied, and was resolved by orchestrator authorization (disclosed to the owner after the fact) -- not a pre-change owner directive and not a silent agent edit. See the 2026-07-06 owner ratification (relay-delivered) below.

2026-07-06 owner ratification (relay-delivered): on 2026-07-06 the owner (benjmcd) directed -- via the orchestrator handoff for this lane, the same delivery channel as the QFREEZE-2 authorization recorded in the Provenance note above -- to ratify the frozen flood unknown_states = [SOURCE_FAILED, OUTSIDE_COVERAGE, STALE, CONFLICTING] as-is and to correct this ledger's provenance. This owner decision is relay-delivered (orchestrator handoff) and is further evidenced by the owner's review-and-merge of PR #202; no separately-committed owner artifact records it beyond those. It supplies the value-level authority that the 2026-07-06 orchestrator-authored removal lacked. Independent adversarial verification (Codex thread 019f2153) confirmed only the NEGATIVE -- that no pre-change owner value-level directive removed UNKNOWN; it did not (and could not) independently verify this ratification itself. It changes no frozen value (the four-state list is unchanged), no threshold, no scope, no safety posture; P0 remains NOT_RUN (BLOCKED=0 NOT_RUN=21). For the record, the over-attribution was committed twice -- introduced at freeze time (PR #191, 0c53023f) and re-introduced by the 2026-07-06 doc-completeness pass (PR #194, b86f177a) -- and PR #202 rewrote both sentences.
...
owner=benjmcd
authority=owner directive delivered via orchestrator handoff (this session, 2026-07-06); relay-delivered -- no separately-committed owner artifact beyond this handoff record and the owner's review-and-merge of this PR. These are product-direction/scope decisions; they change NO qualification value or status (NC/flood QFREEZE-2 stays FROZEN; Bologna stays blocked at ODP-BOL-001; P0 stays NOT_RUN, BLOCKED=0 NOT_RUN=21).
...
2. External users are never a goal. Private/operator use only. Hosted production, Level 10, OAuth/OIDC, registry publication, billing, external secret-manager, and arbitrary-parcel external intake are out of scope (not future work).
