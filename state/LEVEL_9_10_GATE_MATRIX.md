...
| L9-002 area/select intent flow | `PROVEN_PRIVATE_MVP` | `/ui/`, `/intake`, `/operator-cases`, `/operator-cases/supported-aoi/report`, `/ui/operator-cases/supported-aoi/report`; private-MVP and API tests; custom AOI UI runtime smoke posts fixture GeoJSON through `/ui/intake`; supported-AOI API and UI routes prove selected-county fixture evidence against an existing `area_id` | Keep scope limited to selected counties and explicit fixture/custom AOI proof until jurisdiction readiness passes. |
...
| L10-SEC-007 data retention/deletion policy | `VALIDATE_ONLY` | data-retention catalog/runbook/checker plus repo-local audit purge dry-run/apply proof against isolated DB state | Run policy against hosted data stores before production claim. |
...
The preferred next pursuit is now the Bologna recorded-source pilot path. `BPS-001`
created the blocked pilot-scope authority packet; `BPS-REQ-001` makes its missing
product/AOI/scope decisions actionable through structured request rows before
`BSA-001` source/AOI/right-authority preparation. This priority supersedes generic
...
The next Bologna action is to collect or explicitly block the product, one-AOI,
jurisdiction, evidence-only/rulepack, DS-017-treatment, fixture-boundary,
runtime-boundary, and no-overclaim request rows in
